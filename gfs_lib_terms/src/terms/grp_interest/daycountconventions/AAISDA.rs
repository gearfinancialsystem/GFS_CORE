...
impl TraitDayCountConvention for AAISDA {
    ...
    fn day_count_fraction(&self, start_time: PhantomIsoDatetimeW, end_time: PhantomIsoDatetimeW) -> f64 {
        ...
        if y1 == y2 {
            let basis = if start_time.leap_year() { 366.0 } else { 365.0 };
            return (Self::day_count(&self, start_time, end_time) as f64) / basis;
        }

        let first_basis = if start_time.leap_year() { 366.0 } else { 365.0 };
        let second_basis = if end_time.leap_year() { 366.0 } else { 365.0 };

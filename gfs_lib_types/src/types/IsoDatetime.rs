...
impl IsoDatetime {
    ...
    pub fn last_date_of_month(&self) -> IsoDatetime {
    ...
    pub fn provide(string_map: &HashMap<String, Value>, key: &str) -> Option<IsoDatetime> {
        string_map.get(key).and_then(|s| s.as_string().unwrap().parse::<NaiveDateTime>().ok().map(|dt| IsoDatetime(dt)))
    }

    pub fn provide_rc(string_map: &HashMap<String, Value>, key: &str) -> Option<Rc<IsoDatetime>> {

        string_map.get(key).and_then(|s| s.as_string().unwrap().parse::<NaiveDateTime>().ok()).map(|b| Rc::new(IsoDatetime(b)))
    }

    pub fn provide_vec(string_map: &HashMap<String, Value>, key: &str) -> Option<Vec<IsoDatetime>> {
    ...
    pub fn set_value(&mut self, valuex: &IsoDatetime) {
        *self = valuex.clone();
    }

    pub fn parse_from_string(s: &str, fmt: &str) -> Result<IsoDatetime, String> {

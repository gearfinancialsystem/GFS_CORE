...
pub fn compare_test_results(computed_test_res: &Vec<TestResult>, expected_test_res: &Vec<TestResult>) -> bool {
    ...
    for (cr, er) in computed_test_res.iter().zip(expected_test_res.iter()){
        ...
        let a_payoff = cr.payoff.parse::<f64>().unwrap().mul(1000000.0).floor();
        let b_payoff = er.payoff.parse::<f64>().unwrap().mul(1000000.0).floor();
        let a_prin = cr.notionalPrincipal.parse::<f64>().unwrap().mul(1000000.0).floor();
        let b_prin = er.notionalPrincipal.parse::<f64>().unwrap().mul(1000000.0).floor();
        let a_inte = cr.nominalInterestRate.parse::<f64>().unwrap().mul(1000000.0).floor();
        let b_inte = er.nominalInterestRate.parse::<f64>().unwrap().mul(1000000.0).floor();
        let a_accr = cr.accruedInterest.parse::<f64>().unwrap().mul(1000000.0).floor();
        let b_accr = er.accruedInterest.parse::<f64>().unwrap().mul(1000000.0).floor();
        if

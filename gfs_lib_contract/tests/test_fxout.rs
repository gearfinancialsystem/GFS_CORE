...
#[test]
fn test_fxout_contract_creation() {

    let pathx = "tests/test_sets/actus-tests-fxout-converted.json";
    ...
    for (test_id, _) in test_vec  {
        ...
        fxout.run_schedule(Some(to_date));
        let res = fxout.run_apply(None, true); // a

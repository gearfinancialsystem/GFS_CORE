...
impl ContractType {
    ...
    pub fn apply(events: Vec<ContractEvent<IsoDatetime, IsoDatetime>>, cm: &ContractModel, observer: &DataObserver) -> Result<Vec<ContractEvent<IsoDatetime, IsoDatetime>>, String> {
        ...
        match cm.contract_type.clone().as_str() {
            ...
            "BCS" => BCS::apply(events, cm, observer),
            "CPFL" => CAPFL::apply(events, cm, observer),
            "CEC" => CEC::apply(events, cm, observer),
            "CEG" => CEG::apply(events, cm, observer),
            "CLM" => CLM::apply(events, cm, observer),
            "CSH" => CSH::apply(events, cm, observer),
            "FUTUR" => FUTUR::apply(events, cm, observer),
            "FXOUT" => FXOUT::apply(events, cm, observer),
            "COM" => COM::apply(events, cm, observer),
            "LAM" => LAM::apply(events, cm, observer),
            "LAX" =>LAX::apply(events, cm, observer),
            "NAM" => NAM::apply(events, cm, observer),
            "OPTNS" => OPTNS::apply(events, cm, observer),
            "PAM" => PAM::apply(events, cm, observer),
            "STK" => STK::apply(events, cm, observer),
            "SWAPS" => SWAPS::apply(events, cm, observer),
            "SWPPV" => SWPPV::apply(events, cm, observer),
            "UMP" => UMP::apply(events, cm, observer),
            _ => Err("invalid contract type".to_string())
        }
    }
}

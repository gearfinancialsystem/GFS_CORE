...
use crate::functions::ceg::{POF_PRD_OPTNS, STF_PRD_CEG, POF_FP_CEG, STF_FP_CEG, POF_MD_CEG, STF_MD_CEG, POF_XD_OPTNS, STF_XD_CEG, POF_STD_CEG, STF_STD_CEG};
...
impl CreditEnhancementGuarantee {
    pub fn schedule(
        to: &IsoDatetime,
        model: &ContractModel,
        ...
        if !(model.fee_rate.is_none() || model.fee_rate.unwrap() == 0.0) {
            let start_date = if model.cycle_anchor_date_of_fee.is_none() && model.cycle_of_fee.is_none() {
                ...
                Some(model.purchase_date.as_ref().unwrap().plus_period(&model.cycle_of_fee.as_ref().unwrap()))
            } else {
    ...
    pub fn apply(
        ...
        let maturity = Self::maturity(model);
        let mut events = Self::add_external_xd_event(model, events, observer, &maturity)?;

        let states = Self::init_state_space(model, observer, &maturity)?;
        ...
        match model.guaranteed_exposure {
            ...
            _ => {
                ...
                coverage
                    ...
                    * market_object_codes
                    .iter()
                    .map(|code| observer.state_at(code, time, states, model, true))
                    .sum::<f64>()
            }
        }
    }

...
impl StateTransitionFunctionTrait for STF_PP_PAM {
    fn eval(
        &self,
        time: IsoDatetime, 
        states: &StateSpace,
        ...
        if let Some(value) = new_states.notionalPrincipal.as_deref_mut() {
            *value += prepayment_rate * states.notionalPrincipal.as_deref().unwrap(); // Dereference just once due to as_deref_mut
        }

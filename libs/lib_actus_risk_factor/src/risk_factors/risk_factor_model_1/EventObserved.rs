...
use lib_actus_core::events::EventSequence::EventSequence;
use lib_actus_core::events::EventType::EventType;
use lib_actus_core::state_space::StateSpace::StateSpace;
use lib_actus_core::terms::grp_boundary::BoundaryCrossedFlag::BoundaryCrossedFlag;
use lib_actus_core::terms::grp_contract_identification::ContractID::ContractID;
use lib_actus_core::terms::grp_contract_identification::StatusDate::StatusDate;
use lib_actus_core::terms::grp_counterparty::ContractPerformance::ContractPerformance;
use lib_actus_core::terms::grp_counterparty::NonPerformingDate::NonPerformingDate;
use lib_actus_core::terms::grp_fees::FeeAccrued::FeeAccrued;
use lib_actus_core::terms::grp_interest::AccruedInterest2::AccruedInterest2;
use lib_actus_core::terms::grp_interest::AccruedInterest::AccruedInterest;
use lib_actus_core::terms::grp_interest::InterestCalculationBaseAmount::InterestCalculationBaseAmount;
use lib_actus_core::terms::grp_interest::NominalInterestRate2::NominalInterestRate2;
use lib_actus_core::terms::grp_interest::NominalInterestRate::NominalInterestRate;
use lib_actus_core::terms::grp_notional_principal::Currency::Currency;
use lib_actus_core::terms::grp_notional_principal::InterestScalingMultiplier::InterestScalingMultiplier;
use lib_actus_core::terms::grp_notional_principal::MaturityDate::MaturityDate;
use lib_actus_core::terms::grp_notional_principal::NextPrincipalRedemptionPayment::NextPrincipalRedemptionPayment;
use lib_actus_core::terms::grp_notional_principal::NotionalPrincipal2::NotionalPrincipal2;
use lib_actus_core::terms::grp_notional_principal::NotionalPrincipal::NotionalPrincipal;
use lib_actus_core::terms::grp_notional_principal::NotionalScalingMultiplier::NotionalScalingMultiplier;
use lib_actus_core::terms::grp_notional_principal::TerminationDate::TerminationDate;
use lib_actus_core::terms::grp_settlement::ExerciseAmount::ExerciseAmount;
use lib_actus_core::terms::grp_settlement::ExerciseDate::ExerciseDate;
...
pub struct EventObserved {
    pub event_serie_brute: HashMap<String, Vec<ObservedEventPoint>>,
    pub event_serie: HashMap<String, Vec<ContractEvent<IsoDatetime, IsoDatetime>>>,
    // String correspond au contract ID
}
impl EventObserved {
    pub fn new() -> Self {
        Self {
            event_serie_brute: {
                HashMap::<String, Vec<ObservedEventPoint>>::new()
            },
            event_serie: {
                HashMap::<String, Vec<ContractEvent<IsoDatetime, IsoDatetime>>>::new()
            }
        }
    }
    ...
    pub fn new_from(
        file_path: &str,
        test_case_id: &str,
    ) -> Result<EventObserved, Box<dyn std::error::Error>> {
        ...
        if let JsonValue::Array(events) = events_observed {
            ...
            let res_brute = Self::transmute_to_hashmap1(result.clone());
            let res = Self::transmute_to_hashmap2(
              Self::convert_vec_observed_event_point(result, "USD")?
            );
            // ATTENTION ICI CORRIGER CURRENCY

            Ok(EventObserved {
                event_serie_brute: res_brute,
                event_serie: res
            })
        } else {
    ...
    // extrait les contract ID uniques et cree des dico
    fn transmute_to_hashmap1(v: Vec<ObservedEventPoint>) -> HashMap<String, Vec<ObservedEventPoint>> {
        let mut hstring: HashSet<String> = HashSet::new();
        for e in v.clone() {
            hstring.insert(e.get_contract_id());
        }
        let mut new_hm = HashMap::<String, Vec<ObservedEventPoint>>::new();
        for s in hstring {
            let mut new_vec = Vec::<ObservedEventPoint>::new();
            for el in v.clone() {
                let ci = el.get_contract_id();
                if ci == s {
                    new_vec.push(el);
                }
            }
            new_hm.insert(s, new_vec);
    ...
    // extrait les contract ID uniques et cree des dico
    fn transmute_to_hashmap2(v: Vec<ContractEvent<IsoDatetime, IsoDatetime>>) -> HashMap<String, Vec<ContractEvent<IsoDatetime, IsoDatetime>>> {
        let mut hstring: HashSet<String> = HashSet::new();
        for e in v.clone() {
            hstring.insert(e.contract_id.unwrap().value());
        }
        let mut new_hm = HashMap::<String, Vec<ContractEvent<IsoDatetime, IsoDatetime>>>::new();
        for s in hstring {
            let mut new_vec = Vec::<ContractEvent<IsoDatetime, IsoDatetime>>::new();
            for el in v.clone() {
                let ci = el.get_contract_id().value();
                if ci == s {
                    new_vec.push(el);
                }
            }
